...
\begin{lemma}[TUness with adjoint identity matrix]
  ...
  \leanok
  $A$ is TU iff $\left[ I \mid A \right]$ is.
\end{lemma}

\begin{proof}[Proof sketch]
  \uses{def:code_tu_matrix,lem:code_tu_add_unit_row}
  The matrix $\left[ I \mid A \right]$ can be obtained from $A$ by repeatedly adding a unit row, then transposing and permuting
  the columns. Therefore, we can use induction on the number of added identity rows, making use of
  lemma \ref{lem:code_tu_add_unit_row}, lemma \ref{lem:code_tu_transpose}, and the fact that
  permuting columns preserves determinants of submatrices up to a sign.
\end{proof}
